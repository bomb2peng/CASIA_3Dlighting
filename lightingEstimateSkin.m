function v = lightingEstimateSkin(vertex, tri, tex, valid_bin, img, lambda, M, show, harmonic_order, align)
% % using skin color thresholder to eleminate non-skin areas. THE RESULT IS
% % WORSE THAN NO SKIN DETECTION...
...
img = grayworld(img);   % gray world for white balance
...
YCbCr_pixel = zeros(size(vertex));
for i = 1:nChannels
    temp = img(:,:,i);
    tex_pixel(i,:) = double(temp(ind));     %pb: tex_pixel is each vertex's texture in the image.
    temp = img_YCbCr(:,:,i);
    YCbCr_pixel(i,:) = temp(ind);
end
skin_bin = YCbCr_pixel(2,:)>=77/255 & YCbCr_pixel(2,:)<=127/255 & YCbCr_pixel(3,:)>=133/255 & YCbCr_pixel(3,:)<=173/255;   % 0.302, 0.498, 0.522, 0.678
...
flag = logical(flag);
valid_bin = valid_bin & flag & skin_bin';
